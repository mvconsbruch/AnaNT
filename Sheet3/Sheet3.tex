...
$a \mapsto (1 \mapsto \zeta_n^a)$, where $\zeta_n$ is an $n$-th root of unity. As 
we also have $\hat G \oplus \hat H = \widehat{G \oplus H}$, this shows that there
are isomorphisms $G \cong \hat G$ for \textit{all} finite groups. 
...
It is easily seen that these are exactly the primitive characters we described above.
